...
(function ($) {
	...
	Inputmask.prototype = {
		...
		unmaskedvalue: function (value) {
			return maskScope(
				...
				this.el && this.el.inputmask ? this.el.inputmask.maskset : generateMaskSet( this.opts, this.noMasksCache ),
				this.opts
			);
		},
		remove: function () {
			...
			if (optionData !== null) {
				if (typeof optionData === "string") {
					if (option.indexOf( "on" ) === 0) {
						optionData = window[optionData]; // get function definition
	...
	function maskScope(actionObj, maskset, opts) {
			...
			function trackbackAlternations(originalPos, newPos) {
				...
				for (var ps = originalPos; ps < newPos; ps++) {
					if (getMaskSet().validPositions[ps] === undefined && ! isMask( ps, true )) {
						...
						$.each(
							tests,
							function (ndx, tst) { // find best matching
								for (var i = 0; i < tll; i++) {
		...
		function clearOptionalTail(buffer) {
			...
			for (; lmib > rl; lmib--) {
				if (isMask( lmib )) {
					break; // fixme ismask is not good enough
				}
			}
			...
			function generalize() {
				if (opts.keepStatic) {
					...
					if (lastAlt > -1) {
						...
						getMaskSet().validPositions = $.extend( true, {}, positionsClone ); // restore original positions
					}
		...
		function keydownEvent(e) {
			...
			if (k === Inputmask.keyCode.BACKSPACE || k === Inputmask.keyCode.DELETE || (iphone && k === Inputmask.keyCode.BACKSPACE_SAFARI) || (e.ctrlKey && k === Inputmask.keyCode.X && ! isInputEventSupported( "cut" ))) { // backspace/delete
				...
				if ( ! opts.insertMode && caretPos === getMaskSet().maskLength && ! e.shiftKey) {
					caretPos--;
				}
				...
				if (k) {
					...
					if (writeOut !== false) {
						...
						if (getMaskSet().writeOutBuffer && valResult !== false) {
							...
							if (checkval !== true) {
								setTimeout(
									function () { // timeout needed for IE
		...
		function clickEvent(e) {
			...
			var input = this;
			setTimeout(
				function () { // needed for Chrome ~ initial selection clears after the clickevent
		...
		function cutEvent(e) {
			...
			if (document.execCommand) {
				document.execCommand( "copy" ); // copy selected content to system clipbaord
			}
		...
		function blurEvent(e) {
			...
			if (input.inputmask) {
				...
				if (undoValue !== buffer.join( "" )) {
					setTimeout(
						function () { // change event should be triggered after the other buffer manipulations on blur
							$input.trigger( "change" );
