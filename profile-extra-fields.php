...
if ( ! function_exists( 'prflxtrflds_admin_menu' ) ) {
	function prflxtrflds_admin_menu() {
		bws_general_menu();
		$hook = add_submenu_page( 'bws_panel', __( 'Profile Extra Fields Settings', 'profile-extra-fields' ), 'Profile Extra Fields', 'manage_options', 'profile-extra-fields.php', 'prflxtrflds_settings_page' );
...
if ( ! function_exists ( 'prflxtrflds_init' ) ) {
	function prflxtrflds_init() {
		...
		bws_wp_min_version_check( plugin_basename( __FILE__ ), $prflxtrflds_plugin_info, '3.9' );
...
if ( ! function_exists( 'prflxtrflds_settings' ) ) {
	function prflxtrflds_settings() {
		global $prflxtrflds_field_type_id, $prflxtrflds_options, $prflxtrflds_plugin_info, $prflxtrflds_option_defaults, $wpdb;
		...
		if ( ! isset( $prflxtrflds_options['plugin_db_version'] ) || $prflxtrflds_options['plugin_db_version'] != $db_version ) {
			...
			$column_exists = $wpdb->query( "SHOW COLUMNS FROM `" . $wpdb->base_prefix . "prflxtrflds_fields_id` LIKE 'show_in_register_form'" );
			if ( 0 == $column_exists ) {
				$wpdb->query(
...
if ( ! function_exists( 'prflxtrflds_create_table' ) ) {
	function prflxtrflds_create_table() {
		...
		$sql = "CREATE TABLE IF NOT EXISTS `" . $wpdb->base_prefix . "prflxtrflds_fields_id` (
			...
			`show_in_register_form` tinyint(1) NOT NULL DEFAULT '0',
		...
		$sql = "CREATE TABLE IF NOT EXISTS `" . $wpdb->base_prefix . "prflxtrflds_user_field_data` (
			...
			user_id bigint(20) NOT NULL,
			user_value VARCHAR(255) NOT NULL COLLATE utf8_general_ci,
			UNIQUE KEY (id)
		);";
		/* call dbDelta */
		dbDelta( $sql );
...
if ( ! function_exists( 'prflxtrflds_edit_field' ) ) {
	function prflxtrflds_edit_field() {
		...
		if ( ! empty( $_POST ) ) {
			...
			$field_show_in_register_form = isset( $_POST['prflxtrflds_show_in_register_form'] ) ? 1 : 0;
			...
			$field_options = $wpdb->get_row( $wpdb->prepare( "SELECT `field_name`, `required`, `description`, `show_default`, `show_always`, `field_type_id`, `show_in_register_form` FROM `" . $wpdb->base_prefix . "prflxtrflds_fields_id` WHERE `field_id`=%d", $field_id ), ARRAY_A );
			if ( ! $field_options ) {
				...
				$field_show_in_register_form = $field_options['show_in_register_form'];
				/* Get avaliable roles */
			...
			if ( empty( $error ) ) {
				...
				$wpdb->replace(
					...
					array(
						...
						'show_in_register_form' => $field_show_in_register_form
					)
				);
				...
				if ( '1' == $field_type_id || '5' == $field_type_id || '6' == $field_type_id || '7' == $field_type_id || '8' == $field_type_id || '9' == $field_type_id ) {
					if ( '1' == $field_type_id || '8' == $field_type_id )
						$value_name = $field_maxlength;
					elseif ( '9' == $field_type_id )
		...
		if ( empty( $field_id ) ) {
			$action = '?page=profile-extra-fields.php&amp;edit=1';
		} else {
			$action = sprintf( '?page=profile-extra-fields.php&amp;edit=1&amp;prflxtrflds_field_id=%d', $field_id );
		} ?>
		<form class="bws_form" method="post" action="<?php echo $action; ?>">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							...
							<label for="prflxtrflds-required"><?php _e( 'Enable to make this field required.', 'profile-extra-fields' ); ?></label>
						</td>
					</tr>
					<tr>
						...
						<td>
							...
							<label for="prflxtrflds-show-default"><?php _e( 'Show this field by default in User Data. You can change it using Screen Options tab', 'profile-extra-fields' ); ?></label>
						</td>
					</tr>
					<tr>
					...
					<?php if ( ! is_multisite() ) {?>
						<tr>
							...
							<td>
								<input type="checkbox" id="prflxtrflds-show-in-register-form" name="prflxtrflds_show_in_register_form" value="1" <?php checked( ! empty( $field_show_in_register_form ) ); ?> />
								<label for="prflxtrflds-show-in-register-form"><?php _e( 'Show this field in user registration form.', 'profile-extra-fields' ); ?></label>
							</td>
						</tr>
					<?php }?>
					<tr>
...
if ( ! function_exists( 'prflxtrflds_settings_page' ) ) {
	function prflxtrflds_settings_page() {
		global $wpdb, $prflxtrflds_options, $prflxtrflds_plugin_info, $prflxtrflds_option_defaults;
								...
								<tr>
									...
									<td>
										<label>
											...
											<?php _e( 'Enable to show the field if the value is not filled by a user', 'profile-extra-fields' ); ?>
										</label>
									</td>
								</tr>
								<tr>
	...
	function prflxtrflds_show_data( $param ) {
				...
				if ( is_array( $user_ids ) ) {
					...
					foreach ( $user_ids as $user_id ) {
						...
						if ( ! is_numeric( $user_id ) || ! get_user_by( 'id', intval( $user_id ) ) ) {
							...
							$error_message = sprintf( __( 'User with entered id ( id=%s ) does not exist!', 'profile-extra-fields' ), esc_html( $user_id ) );
							break;
						}
					}
				}
		...
		if ( ! empty( $param['user_role'] ) ) {
			...
			if ( is_array( $user_roles ) ) {
				...
				if ( empty( $user_ids ) )
					$error_message = sprintf( __( 'There are no users for the selected roles ( %s )', 'profile-extra-fields' ), esc_html( $param['user_role'] ) );
			}
		}
		/* Get display options */
			...
			if ( ! empty( $_POST['prflxtrflds_user_field_value'] ) ) {
				...
				if ( '2' == $one_entry['field_type_id'] ) {
					...
					$all_entry[ $key ]['user_value'] = $wpdb->get_var( $wpdb->prepare( "SELECT `user_value` FROM `" . $wpdb->base_prefix . "prflxtrflds_user_field_data`
							WHERE `user_id`= %s AND `field_id` = %s", $userid, $one_entry['field_id'] ) );
				}
			}
			...
			if ( 'profile.php' != $hook_suffix ) {
				...
				$editable_visible = $wpdb->get_row( $wpdb->prepare( "SELECT
					...
					WHERE
						...
						AND `" . $wpdb->base_prefix . "prflxtrflds_roles_id`.`role` IN ( '" . $current_user_role . "' )",
					$one_entry['field_id']
				), ARRAY_A );

				$all_entry[ $key ]['editable'] = $editable_visible['editable'];
				$all_entry[ $key ]['visible'] = $editable_visible['visible'];
			}
		}

		if ( 0 < sizeof( $all_entry ) ) {
			/* Render user data */
			$visible_entry_count = false;
			foreach ( $all_entry as $one_entry ) {
				if ( 1 == $one_entry['visible'] ) {
					$visible_entry_count = true;
					break;
				}
			}
			...
			if ( $visible_entry_count > 0 ) {
				...
				<?php echo $hidden_nonvisible_field;
			}
		}
	}
}
		...
		if ( ! empty( $_POST['prflxtrflds_user_field_max_number'] ) ) {
			foreach ( $_POST['prflxtrflds_user_field_max_number'] as $field_id => $max_number ) {
				if ( ! empty( $_POST['prflxtrflds_user_field_value'][ $field_id ] ) && ! in_array( $field_id, $required_array ) ) {
					...
					if ( $max_number > 0 && intval( $_POST['prflxtrflds_user_field_value'][ $field_id ] ) > $max_number )
...
if ( ! function_exists( 'prflxtrflds_shortcode_button_content' ) ) {
	function prflxtrflds_shortcode_button_content( $content ) {
		...
		if ( empty( $prflxtrflds_options ) )
			$prflxtrflds_options = prflxtrflds_settings(); ?>
		<div id="prflxtrflds" style="display:none;">
			<fieldset>
				<label>
					...
					<span class="title"><?php _e( 'Data Rotation', 'profile-extra-fields' ); ?></span>
				</label><br/>
				<label>
			...
			<script type="text/javascript">
				function prflxtrflds_get_shortcode() {
					(function($) {
						var header = $( '.mce-reset #prflxtrflds_header_table option:selected' ).val();
						if ( $( '.mce-reset #prflxtrflds_specify_role' ).is( ':checked' ) ) {
							var user_role = $( '.mce-reset #prflxtrflds_user_role option:selected' ).map(function(){ return this.value.replace(/ /gi, '_'); }).get().join(",");
						} else if ( $( '.mce-reset #prflxtrflds_specify_user' ).is( ':checked' ) ) {
							var user = $( '.mce-reset #prflxtrflds_user option:selected' ).map(function(){ return this.value }).get().join(",");
						} else if ( $( '.mce-reset #prflxtrflds_specify_current' ).is( ':checked' ) ) {
				...
				function prflxtrflds_shortcode_init() {
					(function($) {
						$( '.mce-reset input[name="prflxtrflds_specify"]' ).on( 'change', function() {
							$( '.mce-reset #prflxtrflds_user_role' ).hide();
							$( '.mce-reset #prflxtrflds_user' ).hide();

							if ( $( '.mce-reset #prflxtrflds_specify_user' ).is( ':checked' ) ) {
								if ( $( '.mce-reset #prflxtrflds_user_loader' ).length > 0 ) {
									$( '.mce-reset #prflxtrflds_user_loader' ).show();
									$.ajax({
										url: ajaxurl,
										type: "POST",
										...
										success: function( result ) {
											$( '.mce-reset #prflxtrflds_user' ).html( result );
											$( '.mce-reset #prflxtrflds_user' ).show();
											$( '.mce-reset #prflxtrflds_user_loader' ).remove();
										},
							...
							} else if ( $( '.mce-reset #prflxtrflds_specify_role' ).is( ':checked' ) ) {
								if ( $( '.mce-reset #prflxtrflds_role_loader' ).length > 0 ) {
									$( '.mce-reset #prflxtrflds_role_loader' ).show();
									$.ajax({
										url: ajaxurl,
										type: "POST",
										...
										success: function( result ) {
											$( '.mce-reset #prflxtrflds_user_role' ).html( result );
											$( '.mce-reset #prflxtrflds_user_role' ).show();
											$( '.mce-reset #prflxtrflds_role_loader' ).remove();
										},
										error: function( request, status, error ) {
											console.log( error + request.status );
										}
						...
						$( '.mce-reset #prflxtrflds_header_table, ' + '.mce-reset #prflxtrflds_user_role, ' + '.mce-reset #prflxtrflds_user' ).on( 'change', function() {
							prflxtrflds_get_shortcode();
						});
					})(jQuery);
				}
...
if ( ! function_exists( 'prflxtrflds_get_fields' ) ) {
	function prflxtrflds_get_fields( $args = array() ) {
		...
		if ( false !== $args['required'] ) {
			$where .= "AND `" . $wpdb->base_prefix . "prflxtrflds_fields_id`.`required`='" . absint( $args['required'] ) . "' ";
		}
		...
		$sql_query = "SELECT DISTINCT
			...
			`" . $wpdb->base_prefix . "prflxtrflds_fields_id`.`show_in_register_form`,
...
if ( ! function_exists( 'prflxtrflds_get_field_html' ) ) {
	function prflxtrflds_get_field_html( $field_data = array(), $name = 'prflxtrflds_user_field_value', $echo = false ) {
			...
			if ( is_array( $field_data['available_fields'] ) ) {
				foreach ( $field_data['available_fields'] as $key => $checkbox_data ) {
					$html .= sprintf(
						...
						$editable_attr,
						$required_attr,
						checked( !empty( $value ) && isset( $_POST[ $name ][ $field_data['field_id'] ][ $key ] ), true, false ),
			...
			if ( is_array( $field_data['available_fields'] ) ) {
				foreach ( $field_data['available_fields'] as $key => $radio_data ) {
					$html .= sprintf(
						...
						checked(
									!empty( $value )
									&& isset( $_POST[ $name ][ $field_data['field_id'] ] )
									&& ( $_POST[ $name ][ $field_data['field_id'] ] == $radio_data['value_id'] ),
								true,
								false ),
		...
		if ( 'select' == $field_types[ $field_data['field_type_id'] ] ) {
			if ( is_array( $field_data['available_fields'] ) ) {
				$html = sprintf(
					'<select name="%1$s[%2$s]"><option></option>',
					$name,
					$field_data['field_id']
				);
				foreach ( $field_data['available_fields'] as $key => $option_data ) {
					$html .= sprintf(
						...
						selected(
									!empty( $value )
									&& isset( $_POST[ $name ][ $field_data['field_id'] ] )
									&& ( $_POST[ $name ][ $field_data['field_id'] ] == $option_data['value_id'] )
									,
								true,
								false ),
				...
				<table class="form-table">
					<?php foreach ( $all_entry as $one_entry ) {
						if (
							...
							empty( $one_entry['show_in_register_form'] )
						) {
							...
							<tr>
								...
								<p>
									...
									<br />
									<?php prflxtrflds_get_field_html( $one_entry, 'prflxtrflds_user_field_value', true ); ?>
								</p>
								<br />
							</tr>
						<?php }
